...
# Magic link request valid email -> 200
STATUS=$(curl -s -o /dev/null -w "%{http_code}" -X POST "${BASE_URL}/api/auth/magic-link" \
  -H "Content-Type: application/json" \
  -d '{"email":"smoke-parent@example.com"}')
check "POST /api/auth/magic-link with valid email returns 200" "200" "${STATUS}"

# Magic link request invalid email -> 400
STATUS=$(curl -s -o /dev/null -w "%{http_code}" -X POST "${BASE_URL}/api/auth/magic-link" \
  -H "Content-Type: application/json" \
  -d '{"email":"not-an-email"}')
check "POST /api/auth/magic-link with invalid email returns 400" "400" "${STATUS}"

# Verification invalid token -> 307 redirect to error view
STATUS=$(curl -s -o /dev/null -w "%{http_code}" "${BASE_URL}/api/auth/verify?token=INVALID_RAW_TOKEN_9999")
check "GET /api/auth/verify with invalid token returns 307" "307" "${STATUS}"
